...
dataset = pd.read_csv('Online_hours.csv', index_col=['Time'], parse_dates=['Time'])
plotly_df(dataset, title = "Online users")

def moving_average(series, i):
    return np.average(series[-i:])


def moving_average_visual(series, i):

    rolling_mean = series.rolling(window=i).mean()

    plt.figure(figsize=(20,10))
    plt.title("Moving average\i window size = {}".format(i))
    plt.plot(rolling_mean, "g", label="Trendline")

    plt.plot(dataset[i:], label="Values")
...
def weighted_average (series, weights):
  ...
  for i in range (len(weights)):
    resulted += series [-i-1:]*weights[n]
  return resulted

def exponential_smoothing(series, alpha):
    resulted = [series[0]]
    for i in range(1, len(series)):
        resulted.append(alpha * series[i] + (1 - alpha) * result[i-1])
    return resulted
    ...
    def triple_exponential_smoothing(self):
        ...
        for i in range(len(self.series)+self.n_preds):
            ...
            if i >= len(self.series):
                ...
                self.result.append((smooth + m*trend) + seasonal[i%self.slen])
...
def holtwinters_visual():
    Anomalies = np.array([np.NaN]*len(data))
    Anomalies[data.values<model.LowerBond] = data.values[data.values<model.LowerBond]
    plt.figure(figsize=(20, 10))
    plt.plot(model.result, label = "Model")
    plt.plot(model.UpperBond, "r--", alpha=0.5, label = "Confidence")
    plt.plot(model.LowerBond, "r--", alpha=0.5)
    plt.fill_between(x=range(0,len(model.result)), y1=model.UpperBond, y2=model.LowerBond, alpha=0.5, color = "grey")
    plt.plot(data.values, label = "Actual")
    plt.plot(Anomalies, "o", markersize=10, label = "Anomalies")
    plt.axvspan(len(data)-128, len(data), alpha=0.5, color='green')
    plt.grid(True)
    plt.axis('tight')
...
def process_plotting(n_samples=1000, rho=0):
...
def ts_plot(y, lags=None, figsize=(12, 8), style='bmh'):
...
print("Box-Cox optimal parameter: %f" % lmbda)
...
data["arima_model"] = inv_boxcox(best_model.fittedvalues, lmbda)
forecast = inv_boxcox(best_model.predict(start = data.shape[0], end = data.shape[0]+100), lmbda)
